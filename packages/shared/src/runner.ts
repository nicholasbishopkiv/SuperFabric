...
/**
 * Where the server's runner socket is bind-mounted inside a container, and the socket's filename.
 *
 * **The transport is a unix socket, not a TCP port**, and that is the load-bearing decision of this
 * whole leg. Three things follow from it, in order of how much they matter:
 *
 * 1. **It needs nothing from the host's network stack.** The bridge-gateway route this was originally
 *    planned around does not work on a machine whose host firewall drops `docker0` — a `ufw` default
 *    on Debian/Ubuntu/Arch — and the fix for that is a rule only the machine's operator may add. A
 *    socket on a bind mount sidesteps the question: it is a file, and the container has it.
 * 2. **It removes a listener rather than adding one.** SuperFabric is a local privileged tool that
 *    binds `127.0.0.1` on purpose (see `server/src/origin.ts`); a runner port would have had to be
 *    reachable from the bridge, i.e. from every container on the machine and, on a permissive
 *    firewall, from the LAN. There is no such port.
 * 3. **The egress firewall can stay strict.** The container never needs a route back to the host at
 *    all, so nothing has to be carved out of the allow-list for us.
 *
 * The **directory** is mounted, not the socket file, and read-only. The directory because the server
 * re-creates the socket inode on every boot and a file mount would pin the container to the dead one
 * — mounting the directory is what lets a container that outlived a server restart reconnect to the
 * new listener at the same path. Read-only because connecting to a socket needs write permission on
 * the *socket*, never on the directory holding it, so a contained agent cannot unlink or shadow the
 * factory's socket.
 *
 * Filesystem permissions are the first gate (the socket is `0600`, owned by the user the server runs
 * as) and the per-container token is the second: the first stops another user, the second stops
 * another *container of the same user* claiming a session that is not its own.
 */
export const RUNNER_SOCKET_DIR = "/superfabric";
export const RUNNER_SOCKET_FILE = "runner.sock";

/**
 * The URL a runner dials to reach a socket mounted at `RUNNER_SOCKET_DIR`.
 *
 * `ws+unix://<socket path>:<http path>` is the form Bun's `WebSocket` client understands (and `ws`'s,
 * on Node), which is why the runner needed no code of its own for this: the transport is entirely a
 * property of the URL `ContainerExecutor` puts in the environment.
 */
export function runnerUnixUrl(
  socketDir: string = RUNNER_SOCKET_DIR,
  file: string = RUNNER_SOCKET_FILE,
): string {
  return `ws+unix://${socketDir}/${file}:${RUNNER_WS_PATH}`;
}
...
export const RUNNER_ENV = {
  /**
   * The **attachment** this runner speaks for: one executor run, generated by `ContainerExecutor`
   * and checked by the server on `hello`.
   *
   * Deliberately not SuperFabric's session id, though it stands in the `sessionId` field of the
   * protocol. One session is restarted several times over its life (`set_autonomy`, `set_model`,
   * `set_account`, `set_role`, a pause and a resume), and each restart is a *new* container resuming
   * the same provider conversation; keying the attachment on the session row would mean the outgoing
   * container and the incoming one both claim the same identity for the seconds they overlap. An id
   * per run cannot collide, and it is opaque to the runner either way.
   */
  sessionId: "SUPERFABRIC_SESSION_ID",
  /**
   * Full WebSocket URL of the server. Normally the unix-socket form (`runnerUnixUrl()`); a TCP URL
   * such as `ws://host.docker.internal:4620/runner` is the documented fallback.
   */
